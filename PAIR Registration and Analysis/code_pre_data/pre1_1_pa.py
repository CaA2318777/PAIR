...
for mice_num in mice_nums:
    ...
    # user_input = input("\nWarning! \n是否要重新提取 "+str(mice_num)+" 数据？\n注意这样之前的ANTS所有配准将失效！(Y/N): ")
    ...
    mat_data = loadmat(datapath+'recondata_cut.mat')
    pa_raw = mat_data['recondata_cut']
    pa_raw = np.transpose(pa_raw,(2,0,1))
    pa_raw = np.flip(pa_raw,axis=0)
    pa_raw = np.flip(pa_raw,axis=1)
    pa_raw = pa_raw*mask - (1-mask)*10
    ...
    nib.Nifti1Image(pa_raw,np.eye(4)).to_filename(datapath + 'recondata_cut.nii.gz')
    ...
    print('\nSave '+str(mice_num)+' recondata to nii\n')
    ...
    fixed = ants.image_read(path_before_hd6 + '/xxx/xxx/Data/2024/241113_final_brains/padded_reference_atlas/atlas_75_raw.nii')
    moving = ants.image_read(datapath + 'recondata.nii.gz')
    # moving_real =  ants.image_read(datapath + 'recondata_cut_bgb_unsmooth.nii.gz')
    moving_real =  ants.image_read(datapath + 'recondata_cut.nii.gz')
    ...
    mytx['fwdtransforms'].append(datapath + 'pa_rigid_Generic_Affine_jcy.mat')
    ...
    warped_pa_75 = warped_pa_75[xstart:xend, ystart:yend, zstart:zend]
    nib.Nifti1Image(warped_pa_75,np.eye(4)).to_filename(datapath + 'pa_bg_75um.nii.gz')


    if ifprotein:
        moving_res_ad = ants.image_read(datapath + 'res_ad.nii.gz')
        warped_res_ad = ants.apply_transforms(fixed=fixed, moving=moving_res_ad, transformlist=mytx['fwdtransforms'],
                                        interpolator="linear")
        warped_res_ad = warped_res_ad.numpy()
        warped_res_ad = warped_res_ad[xstart:xend, ystart:yend, zstart:zend]
        nib.Nifti1Image(warped_res_ad,np.eye(4)).to_filename(datapath + 'res_ad_75um.nii.gz')
    ...
    # # 获取所有颜文字名称列表
    # emoji_list = [
    #     "shrug", "table flip", "look of disapproval", "happy face", "sad face",
    #     "kirby", "heart", "bear face", "cat face", "dog face", "monkey face",
    #     "fish", "butterfly", "flower", "coffee", "pizza", "beer", "cake",
    #     "music", "phone", "computer", "lightning", "sun", "moon", "star",
    #     "money", "thumbs up", "thumbs down", "ok hand", "victory hand",
    #     "writing hand", "clapping hands", "waving hand", "robot face"
    # ]

    # # 随机选择一个颜文字并打印
    # print(art(random.choice(emoji_list)))
    # print('\n')
