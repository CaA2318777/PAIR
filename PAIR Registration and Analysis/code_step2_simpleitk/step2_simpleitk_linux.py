...
for mice_num in mice_nums:
    ...
    if ifresult:
        print('Also register Protein Signal')
    else:
        print('No Protein Signal')
    ...
    savepath = datapath+'step2_'+method1+'_simpleitk/'

    # 加载固定图像和移动图像
    fixed_image = sitk.ReadImage(datapath+'step1_'+method1+'/'+method1+'_warped_atlas_25um.nii.gz', sitk.sitkFloat32)
    moving_image = sitk.ReadImage(datapath+'origin/ims_data_25um.nii', sitk.sitkFloat32)
    ...
    sitk.WriteImage(
        affine_moving_resampled, savepath+'simpleitk_affine_warped_ims_25um.nii.gz'
    )
    ...
    if ifresult:
        ...
        sitk.WriteImage(
            affine_moving_mask_resampled, savepath+'/simpleitk_affine_warped_ims_result_25um.nii.gz'
        )
    ...
    sitk.WriteImage(registered_image, savepath+'/' + method1+ '_simpleitk_warped_ims_25um.nii.gz')
    ...
    if ifresult:
        ...
        sitk.WriteImage(
            registered_image_mask, savepath+'/' + method1+ '_simpleitk_warped_ims_result_25um.nii.gz'
        )
    ...
    # 将损失值保存到文本文件
    with open(savepath+'/loss.txt', "w") as file:
    ...
    plt.legend()
    plt.savefig(savepath+"output.png")  # 保存为 PNG 格式
    plt.close()