...
for mice_num in mice_nums:
    ...
    savepath1 = path_before_hd6+'/xxx/xxx/Data/2024/240926_align/' + str(mice_num) + '/'+str(resolution) +'um/'+ '/step1_' + method1 + '/'
    ...
    ############## Load Image  #############
    fixed = ants.image_read(savepath1 + method1 + '_warped_atlas_' + str(resolution) +'um.nii.gz')
    fixed_anno = ants.image_read(savepath1 + method1 + '_warped_anno_' + str(resolution) +'um.nii.gz')
    ...
    shutil.copyfile(mytx['fwdtransforms'][1], savepath2 + 'ims_to_'+method1+'_warped_atlas_Generic_Affine_up.mat')
    shutil.copyfile(mytx['fwdtransforms'][0], savepath2 + 'ims_to_'+method1+'_warped_atlas_Warp.nii.gz')
    shutil.copyfile(mytx['invtransforms'][0], savepath2 + 'ims_to_'+method1+'_warped_atlas_Generic_Affine_up_invtransforms.mat')
    shutil.copyfile(mytx['invtransforms'][1], savepath2 + 'ims_to_'+method1+'_warped_atlas_Warp_invtransforms.nii.gz')
    T2 = time.time()
    ...
    ############## Save Data  #############
    print('Start Saving')
    savemat(savepath2 + method1 +'_ants_warped_ims_' + str(resolution) +'um.mat',{'warped_ims': warped_ims})
    nib.Nifti1Image(warped_ims,np.eye(4)).to_filename(savepath2 +  method1 +'_ants_warped_ims_' + str(resolution) +'um.nii.gz')

    if ifprotein:
        savemat(savepath2 + method1 +'_ants_warped_ims_result_' + str(resolution) +'um.mat',{'warped_ims_result': warped_ims_result})
        nib.Nifti1Image(warped_ims_result,np.eye(4)).to_filename(savepath2 +  method1 +'_ants_warped_ims_result_' + str(resolution) +'um.nii.gz')

    nib.Nifti1Image(warped_back_atlas,np.eye(4)).to_filename(savepath2 +  method1 +'_ants_warped_back_atlas_' + str(resolution) +'um.nii.gz')
    nib.Nifti1Image(warped_back_anno,np.eye(4)).to_filename(savepath2 +  method1 +'_ants_warped_back_anno_' + str(resolution) +'um.nii.gz')
