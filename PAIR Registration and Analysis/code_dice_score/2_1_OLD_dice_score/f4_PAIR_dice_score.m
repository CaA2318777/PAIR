function f4_PAIR_dice_score(rootpath,nums,step1,step2,region)
%F4_PAIR_DICE_SCORE 此处显示有关此函数的摘要
%   此处显示详细说明


    clearvars -except rootpath nums step1 step2 region
    ...
    for num = case_numbers
        disp(['PAIR_dice_score: ',num2str(num),' ', step1,'_',step2,' ',region])
        ...
        for area_num = 1:length(Area_name)
            ...
            save([mat_path,num2str(num),'_',num2str(resolution),'_',char(Area_name(area_num)),'_dice_scores_',step1,'_',step2,'.mat'], 'dice_scores', 'jsc_scores');
            close all;
        end 
    end
