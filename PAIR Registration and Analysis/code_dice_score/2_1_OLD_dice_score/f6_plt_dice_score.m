function f6_plt_dice_score(rootpath,nums,step1,step2,region)
%F6_PKT_DICE_SCORE 此处显示有关此函数的摘要
%   此处显示详细说明
    ...
    for case_num = case_numbers
        ...
        output_img_file = fullfile(img_output_dir, ...
            sprintf('%d_%s_oldmark.jpg', case_num, region));
        ...
        % 数据处理：统计区域和方法数量
        area_count = sum(cellfun(@(x) strlength(x) <= 4, column_names));
        ...
        % 自定义颜色设置
        colors = [hex2rgb('#025259'); hex2rgb('#F29325'); hex2rgb('#C08E7A')];
        ...
        xlabel('Methods');
        ylabel('Value');
        title(['Region-wise Median Dice Score ', num2str(case_num)]);
        xtickangle(45);
        grid on;
        ...
        backup_img_file = fullfile(rootpath, 'img', ...
            sprintf('%d_%s_oldmark.jpg', case_num, region));
