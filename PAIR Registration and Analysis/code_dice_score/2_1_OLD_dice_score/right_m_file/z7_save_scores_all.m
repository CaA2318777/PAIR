...
% 参数设置
resolution = 25;
case_numbers =   [363,358,366];
methods = {'simpleitk', 'ants_simpleitk', ' '};
methods = {'clearmap', 'ants_clearmap', ' '};
methods = {'mba_global', 'ants_mba_global', ' '};
methods = {'amap', 'vm_amap', ' '};
% methods = {'ants', 'vm_ants', ' '};
% methods = {'mba', 'ants_mba', ' '};
methods = {'miracl', 'vm_miracl', ' '};
region = 'big';
max_entries = 20;
...
for case_num = case_numbers
    ...
    dice_score_dir = fullfile(base_path, 'dice_score', 'mat');
    img_output_dir = fullfile(base_path, 'img');
    output_xlsx_dir = fullfile('F:\lab\align\xxx\', [region, '_old_xlsx']);
    xlsx_filename = fullfile(base_path, ...
        sprintf('%d_%d_dice_scores_old_%s.xlsx', case_num, resolution, region));
    ...
    % 初始化数据存储
    column_names = {};
    dice_scores_matrix = nan(max_entries, length(area_names) * length(methods));
    ...
    for area_idx = 1:length(area_names)
        ...
        for method_idx = 1:length(methods)
            ...
            try
                % 加载对应的 Dice Scores
                mat_file = fullfile(dice_score_dir, ...
                    sprintf('%d_%d_%s_dice_scores_%s.mat', case_num, resolution, area_name, method_name));
                loaded_data = load(mat_file, 'dice_scores');
                dice_scores_matrix(1:length(loaded_data.dice_scores), column_idx) = loaded_data.dice_scores;
                fprintf('加载成功: %s\n', mat_file);
            catch
                % 加载失败处理
                dice_scores_matrix(1, column_idx) = -0.01;
                fprintf('   加载失败: %s\n', mat_file);
            end
    ...
    % 转换为表格并写入 Excel
    dice_scores_table = array2table(dice_scores_matrix, 'VariableNames', column_names);
    writetable(dice_scores_table, xlsx_filename);
