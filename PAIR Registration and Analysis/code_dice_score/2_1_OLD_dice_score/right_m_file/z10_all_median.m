...
region = 'small';
fileNumbers =   [269,270,363,366,347,349,358];
fileNumbers =   [316,317,319,320,321,322,323,324];
brain_type = 'samll-wt';

allMedians = [];
fileNames = {}; 
inputFolder = ['F:\lab\align\xxx\', region, '_old_xlsx\'];


% 自定义颜色（按 method 循环）
colors = [hex2rgb('#025259'); hex2rgb('#F29325'); hex2rgb('#C08E7A')]; % 定义颜色
colors = [hex2rgb('#5F869E'); hex2rgb('#B38E72'); hex2rgb('#C08E7A')]; % 定义颜色
% colors = [hex2rgb('#658E9B'); hex2rgb('#C28A5F'); hex2rgb('#C08E7A')]; % 定义颜色
colors = [hex2rgb('#4A708B'); hex2rgb('#D4A967'); hex2rgb('#C08E7A')]; % 定义颜色
colors = [hex2rgb('#5D8E87'); hex2rgb('#D9B15C'); hex2rgb('#C08E7A')]; % 定义颜色
...
outputFile = [inputFolder, 'column_medians.xlsx'];
...
xlabel('Methods');
ylabel('Value');
title(['Region-wise median Dice score   ', root_method,'  n=',num2str(length(fileNumbers)), '  ',brain_type]);
...
saveas(gcf, [inputFolder, 'oldmark_median_colored_boxplot.jpg']);
...
saveas(gcf, [inputFolder, 'oldmark_median_average_colored_boxplot.jpg']);
