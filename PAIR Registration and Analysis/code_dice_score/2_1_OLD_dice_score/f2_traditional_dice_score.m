function [outputArg1,outputArg2] = f2_traditional_dice_score(rootpath,nums,step,region)
%F2_TRADITIONAL_DICE_SCORE 此处显示有关此函数的摘要
%   此处显示详细说明
    ...
    resolution = 25;
    step_type = step;
    region_type = ['_',region];
    case_numbers =   nums;
    

    for case_num = case_numbers
        disp(['traditional_dice_score: ',num2str(case_num),' ', step,' ',region])
        % 数据路径设置
        base_path = [rootpath, num2str(case_num), '\'];
