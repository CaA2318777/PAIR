...
clear mask
fname=[step2_amap_path,'registered_atlas.tiff'];
info = imfinfo(fname);
...
mask = flip(mask,3);
temp = make_nii(mask);
save_nii(temp,[step2_amap_path,method1,'_amap_warped_ims.nii.gz'])

clear mask
fname=[step2_amap_path,'amap_Atlas_25um.tiff'];
info = imfinfo(fname);
num_images = numel(info); 
for i=1:num_images
    a= imread(fname,i);
...
mask = flip(mask,3);
temp = make_nii(mask);
save_nii(temp,[step2_amap_path,method1,'_amap_warped_atlas.nii.gz'])
